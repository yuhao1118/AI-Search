...
class SA:
    ...
    def nn(self):
        ...
        while len(visited) < self.num_cities:
            ...
            for i in range(0, self.num_cities):
                if i not in visited and self.dist_matrix[city][i] < best_edge_len:
                    best_edge_len = self.dist_matrix[city][i]
                    best_next_city = i
            city = best_next_city
        self.tour = visited
        self.length = self.tour_length(visited)
    ...
    def generate_tour(self):
        ...
        loc_2 = random.randint(loc_1 + 1, self.num_cities - 1) # max < num_cities - 1
        new_tour = self.tour[:loc_1] + \
            list(reversed(self.tour[loc_1:loc_2+1])) + self.tour[loc_2+1:]
    ...
    def new_length(self, loc_1, loc_2, new_tour):
        ...
        if loc_2 == self.num_cities - 1:
            remove = self.dist_matrix[self.tour[loc_1 - 1]][self.tour[loc_1]] + \
                self.dist_matrix[self.tour[loc_2]][self.tour[0]]
            add = self.dist_matrix[new_tour[loc_1 - 1]][new_tour[loc_1]] + \
                self.dist_matrix[new_tour[loc_2]][new_tour[0]]
