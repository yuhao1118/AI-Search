...
class SA:
    ...
    def accept_prop(self, new_length):
        return math.exp(-abs(new_length - self.length) / self.T)
    
    def accept(self, new_tour, new_length):
        ...
        elif random.random() < self.accept_prop(new_length):
            self.tour = new_tour
            self.length = new_length
    ...
    def generate_tour(self):
        ...
        new_tour[loc_1], new_tour[loc_2] = new_tour[loc_2], new_tour[loc_1]
        return new_tour, self.new_length(loc_1, loc_2, new_tour)
    
    # Reduce length computation from O(n) to O(1)
    def new_length(self, loc_1, loc_2, new_tour):
        new_length = self.length
        if loc_1 == 0:
            remove = self.dist_matrix[self.tour[-1]][self.tour[loc_1]] + \
                    self.dist_matrix[self.tour[loc_1]][self.tour[loc_1+1]]
            add = self.dist_matrix[new_tour[-1]][new_tour[loc_1]] + \
                    self.dist_matrix[new_tour[loc_1]][new_tour[loc_1+1]]
            new_length = new_length - remove + add
        else:
            remove = self.dist_matrix[self.tour[loc_1-1]][self.tour[loc_1]] + \
                    self.dist_matrix[self.tour[loc_1]][self.tour[loc_1+1]]
            add  = self.dist_matrix[new_tour[loc_1-1]][new_tour[loc_1]] + \
                    self.dist_matrix[new_tour[loc_1]][new_tour[loc_1+1]]
            new_length = new_length - remove + add
            
        if loc_2 == self.num_cities - 1:
            remove = self.dist_matrix[self.tour[loc_2]][self.tour[0]] + \
                    self.dist_matrix[self.tour[loc_2-1]][self.tour[loc_2]]
            add = self.dist_matrix[new_tour[loc_2]][new_tour[0]] + \
                    self.dist_matrix[new_tour[loc_2-1]][new_tour[loc_2]]
            new_length = new_length - remove + add
        else:
            remove = self.dist_matrix[self.tour[loc_2-1]][self.tour[loc_2]] + \
                    self.dist_matrix[self.tour[loc_2]][self.tour[loc_2+1]]
            add  = self.dist_matrix[new_tour[loc_2-1]][new_tour[loc_2]] + \
                    self.dist_matrix[new_tour[loc_2]][new_tour[loc_2+1]]
            new_length = new_length - remove + add
        return new_length
    ...
    def run(self):
        cur_iter = 0
        while cur_iter < self.max_iter and self.T > 0.0001:
            new_tour, new_length = self.generate_tour()
            self.accept(new_tour, new_length)
            self.T *= self.beta
            cur_iter += 1
        return self.best_tour, self.best_length

sa = SA(dist_matrix, num_cities)
tour, tour_length = sa.run()
